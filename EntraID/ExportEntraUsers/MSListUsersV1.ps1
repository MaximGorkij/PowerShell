<#
.SYNOPSIS
Export Microsoft 365 users via Microsoft Graph API with batch processing.
...
$ExportPath = Join-Path $PSScriptRoot "EntraUsers_$timestamp.csv"
$LogPath = Join-Path $PSScriptRoot "EntraUsersLog.log"
...
Write-Log "=== Starting Entra Users Export v3.1 ==="
...
try {
    ...
    Write-Log "- Application permissions: User.Read.All, AuditLog.Read.All" "ERROR"
    if (-not $SkipEmail) {
...
try {
    $users = Invoke-GraphWithRetry -ScriptBlock {
        Get-MgUser -Property "Id,DisplayName,UserPrincipalName,AccountEnabled,LastPasswordChangeDateTime" -PageSize $BatchSize -All -ErrorAction Stop
    }
...
try {
    ...
    Write-Log "Export completed: $ExportPath" "SUCCESS"
...
if (-not $SkipEmail) {
    try {
        if (Test-GraphConnection) {
            ...
            $subject = "Entra ID Users Report"
            $bodyText = "Hello, please find the Entra ID users report attached (CSV)."
            ...
            Write-Log "Email sent successfully to $recipient with CSV attachment" "SUCCESS"
        }
        else {
