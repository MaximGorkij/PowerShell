...
.DESCRIPTION
    ...
    enumerates their members, and exports all data to an Excel file with separate sheets.
.NOTES
    Version: 1.0
    Author: Your Name
...
try {
    # Start logging
    Write-Log "Starting Windows Share Audit Script"
    ...
    function Get-DirectoryPermissions {
        ...
        try {
            ...
            foreach ($access in $acl.Access) {
                ...
                # Only process AD groups (we'll check if it's a group later)
                $obj = [PSCustomObject]@{
                    Path = $Path
                    Domain = $domain
                    Identity = $name
                ...
                try {
                    ...
                    if ($adObject.ObjectClass -eq "group") {
                        Write-Log "Found AD group with access: $name"
    ...
    try {
        ...
        Import-Csv $membersCsv | Export-Excel @excelParams -WorksheetName "Group Members" | Sort-Object -Unique
