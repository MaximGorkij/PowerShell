<#
.SYNOPSIS
  Velkost LOG suborov databazy v MB
#>
param(
    [string]$Server,
    [string]$Port,
...
$LogFile = "C:\Program Files\Zabbix Agent 2\scripts\mssql_log_size.log"
$SqlCmdExe = "C:\Program Files\Microsoft SQL Server\Client SDK\ODBC\170\Tools\Binn\sqlcmd.exe"
$ServerPort = "$Server,$Port"
$Query = "SET NOCOUNT ON;SELECT SUM(size)*8/1024 FROM sys.master_files WHERE database_id=DB_ID('$Database') AND type_desc='LOG'"

"$(Get-Date) | Log size: DB=$Database" | Out-File $LogFile -Append
